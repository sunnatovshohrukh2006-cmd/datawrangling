                    ...
                    if chart_type == "Scatter Plot" and x_col and y_col:
                        if color_col:
                            for group, data in vis_df.groupby(color_col):
                                ...
                                ax.scatter(data[x_col], data[y_col], label=str(group), s=t_size*10, alpha=0.7, color=c, marker=m_curr)
                        else:
                            ax.scatter(vis_df[x_col], vis_df[y_col], color=primary_color, s=t_size*10, alpha=0.7, marker=m_curr)
                        ...
                        ax.hist(vis_df[x_col].dropna(), bins=hist_bins, color=primary_color, edgecolor='black', alpha=0.7)

                    elif chart_type == "Box Plot" and y_col:
                        if x_col:
                            labels, data_vals = [], []
                            for g_name, g_data in vis_df.groupby(x_col):
                                valid = g_data[y_col].dropna().values
                                if len(valid) > 0:
                            ...
                            bplot = ax.boxplot(vis_df[y_col].dropna(), patch_artist=True)
